...
void VBMicrolensing::cmplx_roots_multigen_nd(complex* roots, complex** poly, int degree, bool polish_roots_after, bool use_roots_as_starting_points) {

	static complex poly2[MAXM];
	static int i, j, n, nl, iter, degreenew, k;
	static double dif0;
	static bool success, br;
	static complex coef, prev;
	...
	for (int i = 0; i < degree; i++) {
		roots[i] = zr_mp[nl - 1][i];
	}

	if (polish_roots_after) {
		for (n = 0; n < degree; n++) {
			cmplx_newton_spec(poly[0], degree, &roots[n], iter, success); // Polish roots with full polynomial
		}
	}
	//
